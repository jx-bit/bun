...
| [#78](https://github.com/jx-bit/bun/pull/78) | [pr78-p3-ci-lane-cleanup.md](pr78-p3-ci-lane-cleanup.md)（P3——CI lane 清理：3 死文件退役（0 运行实证，净删 823 行）+ 两测试 lane 去定时（runner 24h 等待 / 5 排除跑全量的结构性红）+ canary 语义修正（常态≠事故，红=评估失败）+ workflows README 职责地图；上游禁用清单因 PAT 无 Actions:write 转交接） | 🔄 OPEN |
