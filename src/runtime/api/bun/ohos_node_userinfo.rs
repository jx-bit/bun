//! OHOS-only: app-sandbox uids have no /etc/passwd and an exec'd `node` child misses the embedded compat shim, so give it a working `os.userInfo()` by injecting a preload via `NODE_OPTIONS` (`--require` a content-hash-named file, username via `BUN_OHOS_USERNAME`). Fail-open everywhere.
...
fn is_disabled(env_array: &[*const c_char]) -> bool {
    if find_env_value(env_array, b"BUN_OHOS_NO_NODE_USERINFO").is_some()
        || bun_core::getenv_z(bun_core::zstr!("BUN_OHOS_NO_NODE_USERINFO")).is_some()
    {
        return true;
    }
    ...
    let shim_disable = find_env_value(env_array, b"OHOS_COMPAT_SHIM_DISABLE").or_else(|| {
        bun_core::getenv_z(bun_core::zstr!("OHOS_COMPAT_SHIM_DISABLE")).map(<[u8]>::to_vec)
    });
...
fn shim_identity() -> &'static ShimIdentity {
    ...
    ONCE.get_or_init(|| {
        ...
        let ret: core::ffi::c_int = loop {
            ...
            if ret == libc::EINTR as core::ffi::c_int {
                continue;
            }
            if ret == libc::ERANGE as core::ffi::c_int {
        ...
        let username = if !pw.pw_name.is_null() {
            ...
            (!bytes.is_empty()
                && !strings::contains_char(bytes, b'=')
                && !strings::contains_char(bytes, 0))
...
fn try_dir(dir_bytes: &[u8], filename: &str) -> Option<ZBox> {
    ...
    match sys::mkdir(&dir_z, 0o700) {
        ...
        Err(e) if e.errno == libc::EEXIST as u16 => match sys::lstat(&dir_z) {
            Ok(st)
...
fn write_all(fd: sys::Fd, mut buf: &[u8]) -> bool {
    while !buf.is_empty() {
        match sys::write(fd, buf) {
            Ok(0) => return false,
            Ok(n) => buf = &buf[n..],
            Err(e) if e.errno == libc::EINTR as u16 => continue,
            Err(_) => return false,
        }
    }
    true
...
mod tests {
    ...
    fn is_disabled_reads_shim_disable_getpwuid_r_from_env_array() {
        ...
        let entries_other: Vec<CString> =
            vec![CString::new("OHOS_COMPAT_SHIM_DISABLE=close_range").unwrap()];
    ...
    fn require_flag_quotes_and_escapes() {
        assert_eq!(
            build_require_flag(b"/tmp/x.cjs"),
            b"--require \"/tmp/x.cjs\""
        );
        assert_eq!(
